...
class PredictiveCodingNet(nn.Module):
    def __init__(self, input_dim, h1_dim, h2_dim, h3_dim, h4_dim,output_dim, lateral_strength=0.1):
        super().__init__()
        ...
        self.W4 = nn.Parameter(torch.randn(h4_dim, h3_dim) * 0.1) 
        self.W5 = nn.Parameter(torch.randn(output_dim, h4_dim) * 0.1)
    ...
    def forward(self, x, num_infer_steps=20, lr_state=0.2):
        ...
        s5 = torch.zeros(batch_size, self.W5.shape[0], device=x.device)
        for _ in range(num_infer_steps):
            ...
            pred_s4 = F.linear(s3, self.W4, self.b4) + torch.matmul(s4, self.L4)
            pred_s5 = F.linear(s4, self.W5, self.b5)
            e1 = s1 - pred_s1
            e2 = s2 - pred_s2
            e3 = s3 - pred_s3
            e4 = s4 - pred_s4
            ...
            s4 = F.relu(s4 - lr_state * e4)
            s5 = F.softmax(s5 - lr_state * e5, dim = 1)
        return s1, s2, s3, s4, s5

    def predict(self, x):
        _, _, _,_, s5= self.forward(x)
        return s5
